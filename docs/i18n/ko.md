...
```text
@mika_qwen  → 현재 대화방을 미카 + Qwen 조합으로 전환 (지속)
@mika_gemma → 같은 대화방을 미카 + Gemma 조합으로 전환 (지속)
```

전환한 뒤 `/friends`를 다시 실행하면 반드시 새 모델이 보여야 합니다. 이전
모델이 계속 보이면 그것은 버그입니다. `bot`으로 끝나는 이름, 긴 문장 속의
`@이름`, 설정에 없는 이름은 절대 라우팅으로 잡히지 않고 일반 메시지로
전달됩니다.
...
여기서 `@이름_qwen_bot`, `@이름_gemma_bot`처럼 **실제 `@botusername` 링크**는
각각 별도의 Telegram 봇 대화방입니다. Gemma 봇을 열면 기존 Qwen 대화가 바뀌는
것이 아니라 Gemma agent의 대화방이 새로 열립니다. 이동한 봇에서 `/friends`를
다시 실행해 `THIS CHAT` 이름·agent·모델을 확인해야 합니다.

별도 대화방이 열리는 것은 이 OpenClaw 다중 봇 방식뿐입니다. Hermes 쪽의
`@이름_qwen`, `@이름_gemma` 같은 `@이름_모델`은 봇 계정이 아니라 같은 대화방
안의 로컬 라우팅 별칭이며, 현재 대화방의 조합만 지속 전환합니다. 두 방식을
혼동하지 마세요.
...
| 같은 대화방에서 이름+모델 조합 전환 | `@이름_모델` (예: `@mika_gemma`) 후 `/friends` 재확인; 대화방 유지, 지속 적용 | `/model`로 현재 봇 대화방의 모델만 변경 |
