...
| --- | --- | --- |
| 전체 친구/업무 라우터 보기 | `/friends` | 이 저장소가 추가하지 않음; 연결된 각 봇 대화방 사용 |
| 성격+모델+도구를 친구용으로 전환 | `/friend` | friend agent에 연결된 봇에 메시지 |
| 성격+모델+도구를 업무용으로 전환 | `/work` | work agent에 연결된 봇에 메시지 |
| 설정된 모델 목록 열기 | `/model` | `/model` 또는 `/model list` |
| 번호로 모델 선택 | Hermes에 표시된 picker 사용 | `/model <번호>` |
| 임시 모델 선택 해제 | 원하는 프로필을 다시 시작 | `/model default` |
| 현재 모델 확인 | `/friends`는 힌트, `/model`이 실제 기준 | `/model status` |
...
그다음 Telegram에서 `/friends` → `/friend` → 새 메시지, 다시 `/work` → 새
메시지 순서로 시험합니다. 두 모드의 말투, 모델 동작, 허용 도구가 의도와 맞으면
연결이 끝난 것입니다.
